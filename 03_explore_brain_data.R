...
fit_model_region_pond <- function(brain_dat, region, group = "cross"){
  ...
  # fit two models, 
  # 1: reduced, only the variables being controllde for), 
  # 2: the full model with treatment effect
  ...
  # perform anova
  model_return <- list(anova(mod1, mod2, mod3))
  names(model_return) <- region
